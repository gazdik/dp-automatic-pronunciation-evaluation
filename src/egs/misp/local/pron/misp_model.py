...
class MispModel(ModelInterface):
    lr_0 = 0.001
    drop = 0.5
    epochs_drop = 1
    ...
    def build(cls, input_dim, out_dim, hidden_act='relu', out_act='softmax'):
        ...
        y = Dense(256, activation=hidden_act)(x)
        y = Dropout(0.2)(y)
        y = Dense(out_dim, activation=out_act)(y)

        m = Model(inputs=x, outputs=y)

        opt = keras.optimizers.Adam(lr=MispModel.lr_0)
    ...
    def train_decay(self, train_generator, validation_generator, out_fname,
        ...
        lrs = LearningRateScheduler(MispModel.step_decay, verbose=1)
    ...
    def step_decay(epoch):
        lr = MispModel.lr_0 \
             * math.pow(MispModel.drop,
                        math.floor((1 + epoch) / MispModel.epochs_drop))

        return lr

    @staticmethod
    def loss(y_true, y_pred):
        y_true_s, y_pred_s = MispModel.mask_invalid(y_true, y_pred)
