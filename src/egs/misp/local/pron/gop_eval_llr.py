...
with open(score_fname, 'wb') as f:
    for utt, feats in feats_it:
        ...
        scores = np.zeros_like(flags, dtype=np.float)
        for i in range(len(flags)):
            # Calc average likelihoods for current segment
            likelihoods_avg = np.average(feats_per_phone[seg_idx[i]:seg_idx[i + 1]], axis=0)

            # The numerator
            num_val = likelihoods_avg[phone_ali[i]]

            # Invalidate values from the numerator
            likelihoods_avg[phone_ali[i]] = float('-inf')

            # Find the second highest value
            den_val = np.nanmax(likelihoods_avg, axis=0)

            # Calc result
            scores[i] = num_val - den_val
