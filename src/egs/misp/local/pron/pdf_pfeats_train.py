...
import os
import argparse
from pdf_pfeats_model import PdfPfeatsModel
from pdf_pfeats_generator import DataGenerator
...
tr_gen = DataGenerator(args.feats_tr, args.pdf_tr, args.phones_tr,
                       args.exp_dir, phn_map, args.lang, tmpdir_tr,
                       batch_size=256)
cv_gen = DataGenerator(args.feats_cv, args.pdf_cv, args.phones_cv,
                       args.exp_dir, phn_map, args.lang, tmpdir_cv,
                       batch_size=256)

if args.base_model:
    raise Exception("Knowledge transfer from a base model isn't implemented yet.")
else:
    m = PdfPfeatsModel.build(tr_gen.input_dim(), tr_gen.output_dim())

# Initial training
m.train(tr_gen, cv_gen, model_fname, epochs=1, patience=5)
