...
ali_free_fname = os.path.join(args.cbps_dir, 'free_ali_test/ali_pdf.gz')
...
ali_free_it = kio.read_ali_ark(ali_free_fname)
...
with open(score_fname, 'wb') as f:
    for flags_t, ali_free_t, ali_force_t, ali_force_frm_t, feats_t in zip(flags_it, ali_free_it, ali_force_it, ali_force_frm_it, feats_it):
        # Unpack each tuple
        utt, flags = flags_t
        _, ali_free = ali_free_t
        _, ali_force = ali_force_t
        _, ali_force_frm = ali_force_frm_t
        _, feats = feats_t

        # Get only features for corresponding states in alignments
        feats_force = hlp.np_pick(feats, ali_force)
        feats_free = hlp.np_pick(feats, ali_free)
        ...
        for i in range(len(flags)):
            ...
            probs_num = feats_force[seg_idx[i]:seg_idx[i + 1]]
            probs_den = feats_free[seg_idx[i]:seg_idx[i + 1]]
            avg_num = np.average(probs_num)
            avg_den = np.average(probs_den)
            scores[i] = np.abs(avg_num - avg_den)
