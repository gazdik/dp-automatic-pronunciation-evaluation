...
phones_tr="ark:ali-to-phones ${exp_dir}/final.mdl "
phones_tr+="\"ark:gunzip -c ${ali_tr_dir}/ali*.gz |\" ark:- |"
phones_cv="ark:ali-to-phones ${exp_dir}/final.mdl "
phones_cv+="\"ark:gunzip -c ${ali_cv_dir}/ali*.gz |\" ark:- |"

# Calculate mean and variance of averaged features
phones_ali_tr_cv="ark:ali-to-phones --write-lengths ${ali_tr_dir}/final.mdl "
phones_ali_tr_cv+="\"ark:gunzip -c ${ali_tr_dir}/ali*.gz ${ali_cv_dir}/ali*.gz |\" ark,t:- |"
apply-cmvn --norm-vars=${norm_vars} --norm-means=${norm_means} \
    ...
    local/utils/avg_feats.py "${phones_ali_tr_cv}" | \
...
apply-cmvn --norm-vars=${norm_vars} --norm-means=${norm_means} \
    ...
    splice-feats ${splice_opts} ark:- ark:- | \
    ${feats_model_dir}/forward.py ${feats_model_dir}/final.h5 | \
    local/utils/avg_feats.py "${phones_ali_tr}" | \
...
apply-cmvn --norm-vars=${norm_vars} --norm-means=${norm_means} \
    ...
    splice-feats ${splice_opts} ark:- ark:- | \
    ${feats_model_dir}/forward.py ${feats_model_dir}/final.h5 | \
    local/utils/avg_feats.py "${phones_ali_cv}" | \
    local/utils/cmvn_transform.py ${exp_dir}/cmvn.mdl > \
