...
mkdir -p ${exp_dir}
ln -frs local/pron/pfeats_forward.py ${exp_dir}/forward.py
...
# Train model
$cmd ${log_dir}/pfeats_train.log local/pron/pfeats_train.py ${train_opts} \
    "${feats_tr}" "${feats_cv}" "${phones_tr}" "${phones_cv}" $pfeats_lang \
    ${exp_dir} ${base_model}
