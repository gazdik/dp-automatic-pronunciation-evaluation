...
if [[ $# -lt 8 ]]; then
    echo "Usage: $0 <lang-dir> <conf-dir> <data> <ali-force> <ali-free> \\"
...
phones_ali_force="ark:ali-to-phones --write-lengths ${exp_dir}/final.mdl "
phones_ali_force+="\"ark:gunzip -c ${force_ali_dir}/ali*.gz|\" ark,t:- |"

phones_per_frame_free="ark:ali-to-phones --per-frame ${exp_dir}/final.mdl "
phones_per_frame_free+="\"ark:gunzip -c ${free_ali_dir}/ali*.gz|\" ark:- |"
...
# Evaluate gop scores
${cmd} ${log_dir}/gop_eval.log local/pron/gop_eval_std.py \
    "${feats}" "${phones_ali_force}" "${phones_per_frame_free}" \
    ${exp_dir}/phn_to_pdf.int ${flags} ${eval_dir}

# Create alignments in the text grid format
${cmd} ${log_dir}/align2ctm.log \
    ali-to-phones --ctm-output ${exp_dir}/final.mdl \
    "ark:gunzip -c ${force_ali_dir}/ali*.gz |" \
...
local/utils/eq_err_rate.py ${flags} ${eval_dir}/score.ark \
    --thr_start -30.0 --thr_stop 30.0 --trend inc > \
    ${eval_dir}/eer.txt
...
# Plot ROC curve
local/utils/plot_roc.py ${flags} ${eval_dir}/score.ark inc \
