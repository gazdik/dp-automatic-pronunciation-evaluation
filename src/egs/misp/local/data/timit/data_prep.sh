...
#           2019   Brno University of Technology (Author: Peter Gazdik)
# Apache 2.0.
...
# or lower-cased
if [ ! -d $timitdir/TRAIN -o ! -d $timitdir/TEST ] && [ ! -d $timitdir/train -o ! -d $timitdir/test ]; then
...
for x in train dev test; do
  ...
  cat ${x}.trans | $local/data/timit/norm_trans.pl -i - -m $conf/phones.60-48-39.map -to 48 | sort > $x.text || exit 1;
